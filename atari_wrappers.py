...
class RescaleAction(gym.ActionWrapper):
    ...
    def __init__(self, env,length = 2):
        super(RescaleAction, self).__init__(env)
        self._action_set = np.array([0,  3,  4], dtype=np.int32)

        self.action_space = spaces.Discrete(length+1)
...
def process_frame(frame, width, height, normalize=False):
    ...
    # debug()
    return resized_screen.reshape(height, width, 1)
...
class WarpFrame(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        return process_frame(obs, width=self.width, height=self.height)
